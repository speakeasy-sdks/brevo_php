<?php

/**
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

declare(strict_types=1);

namespace test\BREVO\Models\Operations;

use \test\BREVO\Utils\SpeakeasyMetadata;
class UpdateResellerChildRequest
{
    /**
     * Either auth key or id of reseller's child
     * 
     * @var string $childIdentifier
     */
	#[SpeakeasyMetadata('pathParam:style=simple,explode=false,name=childIdentifier')]
    public string $childIdentifier;
    
    /**
     * values to update in child profile
     * 
     * @var \test\BREVO\Models\Shared\UpdateChild $updateChild
     */
	#[SpeakeasyMetadata('request:mediaType=application/json')]
    public \test\BREVO\Models\Shared\UpdateChild $updateChild;
    
	public function __construct()
	{
		$this->childIdentifier = "";
		$this->updateChild = new \test\BREVO\Models\Shared\UpdateChild();
	}
}
